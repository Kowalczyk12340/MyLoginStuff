...
//logout user
router.get("/api/users/logout", authMiddleware, logoutUser);
...
router.put(
  ...
  authMiddleware,
  [
    check("username", "Username is required").isString().isLength("5"),
...
router.post(
  ...
  authMiddleware,
  [
    check("title", "Title is required").isString().isLength({ min: 3 }),
    check("description", "Description is required").isString(),
